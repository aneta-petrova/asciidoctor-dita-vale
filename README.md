...
| LinkAttribute | DITA 1.3 does not allow references to reusable content in link URLs. Remove attribute references from the link or replace the entire [link macro](https://docs.asciidoctor.org/asciidoc/latest/macros/link-macro/) with an attribute reference. |
...
| ShortDescription | DITA 1.3 supports the `<shortdesc>` element to provide a short description of the topic. Assign `[role="_abstract"]` to a paragraph you want to convert to the `<shortdesc>` element. |
